...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        S = Sentence(neigh_list, count)
        self.knowledge.append(S)
        for i in self.knowledge:
            if i.known_mines() != None:
                for m in list(i.known_mines()):
                    self.mark_mine(m)
            if i.known_safes() != None:
                for s in list(i.known_safes()):
                    self.mark_safe(s)
            for j in self.knowledge:
                if i.cells.issubset(j.cells) and len(i.cells) != 0 and len(j.cells) != 0 and i != j:
                    self.knowledge.remove(j)
                    self.knowledge.append(Sentence(j.cells - i.cells, j.count - i.count))
    ...
    def make_random_move(self):
        ...
        """
        i = (random.randint(0, 7), random.randint(0, 7))
